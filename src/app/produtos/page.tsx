...
export default function ProdutosPage() {
  ...
  function submeter() {
    ...
    if (parecidos.length > 0 && !confirmarMesmoAssim) return; // pede confirmaÃ§Ã£o primeiro
    adicionarProduto({
      nome: novo.nome.trim(),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-xl font-semibold">CatÃ¡logo de produtos</h1>
          <p className="text-sm text-slate-500">
            Esta Ã© a lista &ldquo;oficial&rdquo; do que existe (categorias, unidades, etiquetas QR). Mexe-se
            pouco â sÃ³ quando aparece um produto novo. Para ver quanto tÃªm em stock agora e
            registar entradas/saÃ­das do dia a dia, usa a pÃ¡gina <strong>InventÃ¡rio</strong>.
          </p>
        </div>
        <button
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-slate-500 text-left">
              <tr>
                ...
                <th className="px-3 py-2 font-medium">UnidÃªe</th>
                <th className="px-3 py-2 font-medium">Rastreio</th>
                <th className="px-3 py-2 font-medium text-right">Stock mÃ­nimo</th>
              </tr>
            </thead>
      ...
      {podeGerirCatalogo ? (
        <div className="bg-white rounded-lg border border-slate-200 p-4 max-w-lg print:hidden">
          <h2 className="font-medium text-sm mb-3">Adicionar novo produto ao catÃ¡logo</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            {parecidos.length > 0 && (
              <div className="sm:col-span-2 bg-amber-50 border border-amber-200 rounded-md p-3 text-xs text-amber-800 space-y-1">
                <p className="font-medium">JÃ¡ existe produto parecido no catÃ¡logo:</p>
                ...
                <label className="flex items-center gap-2 pt-1">
                  ...
                  />
                  Ã mesmo um produto diferente â criar na mesma
                </label>
              </div>
            )}
            ...
            <input
              type="number"
              min={0}
              placeholder="Stock mÃ­nimo no armazÃ©m"
            ...
            <select
              ...
              className="border border-slate-200 rounded-md px-2 py-1.5 text-sm"
            >
              <option value="unitario">Rastreio unitÃ¡rio (QR = a unidade)</option>
              <option value="lote">Por caixa/lote (QR sÃ³ na caixa)</option>
            </select>
        ...
        <p className="text-xs text-slate-400 print:hidden">
          SÃ³ a gestÃ£o e a administrativa podem adicionar produtos novos ao catÃ¡logo, para evitar
