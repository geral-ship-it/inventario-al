...
export default function InventarioPage() {
  ...
  function handleQr(texto: string) {
    ...
    if (produto) {
      ...
      setMensagem("QR code não corresponde a nenhum produto do catálogo.");
    }
  }
  ...
  function aplicarMovimento(tipo: "entrada" | "saida") {
    ...
    registarMovimento(
      ...
      tipo,
      quantidade,
      motivo || (tipo === "entrada" ? "Reposição manual" : "Saída para apartamento/arrecadação")
    );
    setMensagem(
      `${tipo === "entrada" ? "Entrada" : "Saída"} de ${quantidade} ${produtoSelecionado.unidade}(s) de "${produtoSelecionado.nome}" registada.`
    );
    setQuantidade(1);
    setMotivo("");
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold">Inventário — armazém principal</h1>
        <p className="text-sm text-slate-500">
          Ecrã do dia a dia: quanto têm em stock agora, e onde se regista cada entrada/saída. Lê
          o QR code do produto ou pesquisa manualmente. (Para gerir o catálogo de produtos em si —
          nomes, categorias, etiquetas QR — usa a página <strong>Produtos</strong>.)
        </p>
      </div>
              ...
              <div className="border-t border-slate-100 pt-3 space-y-2">
                <p className="text-sm">
                  ...
                  <span className="text-xs text-slate-400">
                    (mínimo {produtoSelecionado.stockMinimoArmazem})
                  </span>
                </p>
                ...
                {produtoSelecionado.tipoRastreio === "lote" && (
                  <p className="text-[11px] text-amber-600 bg-amber-50 border border-amber-200 rounded-md px-2 py-1.5">
                    ...
                    {produtoSelecionado.unidade}s por caixa. A saída individual não tem QR próprio —
                    usa &ldquo;Nova caixa aberta&rdquo; quando abrires uma, e ajusta a saída à mão quando fizeres
                    a contagem periódica.
                  </p>
                )}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-lg border border-slate-200">
            <div className="px-4 py-2.5 border-b border-slate-100">
              <h2 className="font-medium text-sm">Últimos movimentos</h2>
            </div>
            {movimentos.length === 0 ? (
