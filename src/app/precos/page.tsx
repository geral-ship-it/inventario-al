...
export default function PrecosPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold">PreÃ§os por loja</h1>
        <p className="text-sm text-slate-500">
          Continente, Pingo Doce, Auchan e Lidl tÃªm catÃ¡logo online â pensados para
          atualizaÃ§Ã£o automÃ¡tica periÃ³dica. Makro, Recheio e PoupanÃ§a exigem registo
          prÃ³prio, por isso ficam com inserÃ§Ã£o manual.
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="bg-white rounded-lg border border-slate-200 p-4">
          <h2 className="font-medium text-sm mb-3">Registar / atualizar preÃ§o</h2>
        ...
        <div className="bg-white rounded-lg border border-slate-200">
          <div className="px-4 py-3 border-b border-slate-100">
            <h2 className="font-medium text-sm">
              HistÃ³rico do produto selecionado ({produtos.find((p) => p.id === produtoId)?.nome})
            </h2>
          </div>
          {precosDoProduto.length === 0 ? (
            <p className="p-4 text-sm text-slate-500">Ainda sem preÃ§os registados.</p>
          ) : (
            <ul className="divide-y divide-slate-100">
              {precosDoProduto.map((pr) => (
                <li key={pr.id} className="px-4 py-2 text-sm flex items-center justify-between">
                  <span>
                    {pr.loja} â {formatarPreco(pr.preco)}{" "}
                    {pr.automatico && (
      ...
      <p className="text-xs text-slate-400 max-w-2xl">
        Nota tÃ©cnica: a atualizaÃ§Ã£o automÃ¡tica para Continente, Pingo Doce, Auchan e Lidl vai
        ser feita por uma tarefa agendada que consulta os catÃ¡logos pÃºblicos destas lojas.
        Como os sites mudam com frequÃªncia, esta parte vai precisar de manutenÃ§Ã£o periÃ³dica â
        os preÃ§os manuais continuam sempre disponÃ­veis como alternativa fiÃ¡vel.
      </p>
    </div>
  );
}
