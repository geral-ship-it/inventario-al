...
export default function ComprasPage() {
  ...
  function handleQr(texto: string) {
    ...
    if (!produto) {
      setMensagemQr("QR code não corresponde a nenhum produto do catálogo.");
      return;
    }
    criarItemManualNaListaAberta(produto.id, 1);
    setMensagemQr(`“${produto.nome}” adicionado à lista de compras.`);
  }

  // Carrinho: total estimado por loja, com base na loja escolhida em cada
  // item (ou no preço mais barato conhecido, quando ainda não escolheram).
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">
          Lista quinzenal. Junta pedidos das arrecadações, itens do armazém e itens manuais;
          quando estiver pronta, fecha para ir às compras.
        </p>
      </div>
        ...
        <div className="bg-white rounded-lg border border-slate-200">
                          ...
                          {opcoesPreco
                            ...
                            .map((p) => (
                              <option key={p.loja} value={p.loja}>
                                {p.loja} — {formatarPreco(p.preco)}
                                {barato && p.loja === barato.loja ? " (mais barato)" : ""}
                              </option>
                            ))}
                        </select>
                      ) : (
                        <span className="text-xs text-slate-400">sem preço registado</span>
                      )}
                    </div>
                  </li>
                );
          ...
          {totalGeral > 0 && (
            <div className="px-4 py-3 border-t border-slate-100 bg-slate-50 text-sm space-y-1">
              <p className="font-medium mb-1">Carrinho — total estimado por loja</p>
          ...
          <div className="p-4 border-t border-slate-100 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              >
                <option value="">Adicionar item manualmente…</option>
                {produtos.map((p) => (
      ...
      {listaFechada && (
        <div className="bg-white rounded-lg border border-amber-200">
          <div className="px-4 py-3 border-b border-amber-100 bg-amber-50 flex items-center justify-between">
            <div>
              <h2 className="font-medium">Lista fechada — a aguardar receção</h2>
              <p className="text-xs text-slate-500">
                Fechada em {listaFechada.fechadaEm && formatarData(listaFechada.fechadaEm)}. Marca cada
                item como recebido à medida que a administrativa entrega no armazém.
              </p>
            </div>
            <button
            ...
            >
              Confirmar reposição e dar entrada no stock
            </button>
          </div>
          <ul className="divide-y divide-slate-100">
              ...
              return (
                <li key={it.id} className="px-4 py-2.5 text-sm flex items-center justify-between">
                  <span>
                    {produto?.nome} × {it.quantidade}
                    {it.lojaEscolhida && (
                      <span className="text-xs text-slate-400"> — {it.lojaEscolhida}</span>
                    )}
                  </span>
      ...
      <div className="bg-white rounded-lg border border-slate-200">
        <div className="px-4 py-3 border-b border-slate-100">
          <h2 className="font-medium">Histórico de listas concluídas</h2>
        </div>
        {historico.length === 0 ? (
          <p className="p-4 text-sm text-slate-500">Ainda sem listas concluídas.</p>
        ) : (
          <ul className="divide-y divide-slate-100">
            {historico.map((l) => (
      ...
      <p className="text-xs text-slate-400">
        Lojas configuradas para comparação de preços: {LOJAS.join(", ")}.
      </p>
    </div>
  );
}
